  ...
  function renderOptions(override?: Partial<RenderOptions>): RenderOptions {
    return {
      ...
      createEvaluater() {
        return {
          evaluateProgram() {
            throw new Error('Program evaluation is not allowed');
          },
          evaluateExpression(node) {
            if (node.type === 'Identifier') return override?.components?.[node.name];
            throw new Error(`${node.type} expression is not allowed`);
          },
        };
      },
      ...JsxRuntime,
  ...
  return {
    async compile(input) {
      ...
      return {
        ...
        file,
        render(components) {
          return toJsxRuntime(tree, renderOptions({ filePath: file.path, components }));
        },
      };
    },
